...
Fabric build, Java 32개·Python 107개·Node 22개 및 기존 합성 연결·취소 시험 통과.
제작 증거 함수, 레시피 허용 목록, 구형 클라이언트 거절, 실행 기한과 취소 증거를 자동 검사했다.
실제 서버의 레시피 채우기·슬롯 갱신을 포함한 제작 왕복은 위 실게임 시험으로 확인해야 한다.
사용자가 이전 핫바 선택의 실게임 성공을 확인했으며 제작의 실게임 결과는 아직 없다.
