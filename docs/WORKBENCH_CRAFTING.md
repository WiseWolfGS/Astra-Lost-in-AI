...
수정 모드를 적용하기 위해 게임을 저장·정상 종료하고 다시 실행한다.
...
Fabric build, Java 35개·Python 119개·Node 25개 및 기존 격리 연결·취소 시험을 통과했다.
레시피 재료 규약, 요청 범위, capability 거절, 긴 실행 기한과 응답 증거 보존을 자동 검사했다.
후속 사용자 테스트에서 3×3 제작의 실게임 성공을 확인했다. 모든 취소·지연 조건의 검증 완료를 뜻하지 않는다.
이전 작업대 설치와 2×2 제작은 사용자 테스트 성공을 확인했다.
