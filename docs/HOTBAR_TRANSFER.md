...
수정 모드를 적용하기 위해 월드를 저장하고 게임을 정상 종료한 뒤 다시 실행한다.
...
Java 37개·Python 129개·Node 26개 및 격리 연결·취소 시험을 통과했다.
슬롯 매핑·요청 제한·구형 클라이언트 거절·응답 보존을 자동 검사했다. 후속 사용자 테스트에서 실제 핫바 이동 성공을 확인했다.
이전 작업대 3×3 제작은 사용자가 성공을 확인했다. 이후 돌 재료 확보와 돌 도구 제작을 연결한다.
