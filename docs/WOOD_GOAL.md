...
발 아래 지지 블록은 채굴 대상으로 선택하지 않는다. 경로 실패 시 다른 나무로 재시도하지 않는다.
...
- 통신/시간 제한으로 마지막 행동 종료 여부가 불명확하면 원래 세션에 stop을 요청한다.
  정리 요청 예산은 추가 최대 8초이며, `stopRequest=queued`는 중단 완료 증거가 아니다.
...
최신 검증은 Python 56개, Java 20개 통과 및 Fabric 빌드 성공이다. PowerShell 구문 검사도 통과했다.
이 오류 수정 검증에는 모델 호출을 사용하지 않았다. [수정 보고서](NAVIGATION_FIX.md)를 참조한다.

다음 단계는 실패 기록을 반영해 이 첫 스킬을 안정화하고, 성공·실패 조건이 있는 스킬 저장과 선택을 추가하는 것이다.
